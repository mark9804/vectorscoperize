...
class ScopeRenderer: NSObject, MTKViewDelegate, ObservableObject {
    ...
    func createConfigBuffer() {
        ...
        configBuffer = device.makeBuffer(bytes: &config, length: MemoryLayout<ScopeConstants.MetalConfig>.size, options: .cpuCacheModeWriteCombined)
    }
    ...
    func buildPipelines() {
        ...
        do {
            ...
            do {
                ...
                } else if let sourceURL = Bundle.module.url(forResource: "ScopeShaders", withExtension: "metal") {
                     print("Found ScopeShaders.metal source at \(sourceURL.path), compiling at runtime...")
            ...
            guard let vsFunc = library.makeFunction(name: "vectorscope_accumulate"),
                  ...
                  let clearParadeFunc = library.makeFunction(name: "clear_parade") else {
    ...
    func setInput(publisher: PassthroughSubject<CMSampleBuffer, Never>) {
        publisher
            ...
            .sink { [weak self] buffer in
                ...
                // We should trigger a redraw of the MTKView here if it's paused, 
                // but usually we set isPaused=false and let the display link drive it, 
                // OR we enable enableSetNeedsDisplay = true.
                // For video sync, manual drawing is often better to avoid rendering duplicate frames.
            }
    ...
    func draw(in view: MTKView) {
        ...
        guard let imageBuffer = CMSampleBufferGetImageBuffer(sBuffer),
              ...
              let clearParadeState = clearParadeState else {
            // print("ScopeRenderer: Missing metal components or buffers")
            return
        }
        
        // 1. Create Input Texture from CVPixelBuffer
        guard let inputTexture = makeTextureFromCVPixelBuffer(pixelBuffer: imageBuffer, pixelFormat: .bgra8Unorm) else { return }
        
        // 2. Ensure Output Texture exists
        let drawSize = view.drawableSize
        if outputTexture == nil || outputTexture!.width != Int(drawSize.width) || outputTexture!.height != Int(drawSize.height) {
        ...
        let threadGroups = MTLSize(width: (outTex.width + 15) / 16, height: (outTex.height + 15) / 16, depth: 1)
    ...
    func makeTextureFromCVPixelBuffer(pixelBuffer: CVPixelBuffer, pixelFormat: MTLPixelFormat) -> MTLTexture? {
        ...
        CVMetalTextureCacheCreateTextureFromImage(kCFAllocatorDefault, textureCache, pixelBuffer, nil, pixelFormat, width, height, 0, &cvTextureOut)
